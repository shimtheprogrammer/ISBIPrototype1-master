...
/**
 * Created by MRuto on 2/10/2017.
 */

public class AssetGainsDisplayActivity extends AppCompatActivity
    ...
    private void prepareListData() {
        ...
        List<AssetGains> allPurchases = agdb.getAllAssetGains();

        int count = 0;
        for (AssetGains pd : allPurchases) {
                String log = "Id: " + pd.getId() + " ,Customer: " + pd.getCustomer()
                        + " ,Total: " + numberFormat.format(pd.getTotal())
                        + " ,Payment Method: " + pd.getPayment() + " ,Date: " + pd.getDate()
                        + " ,ForeignKey: " + pd.gettForeignKey();
                Log.d("Insert: ", log);
                purchase.add(log);
                count++;
        }

        listDataHeader.add("Gains/loss on Assets: " + count);
