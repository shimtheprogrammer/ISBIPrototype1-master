...
public class GoodsPreOrderedDisplayActivity extends AppCompatActivity
    ...
    private void prepareListData() {
        ...
        List<String> goodsSale = new ArrayList<String>();

        List<GoodsPreOrdered> allGoods = db.getAllGoodsPreOrders();

        int count = 0;
        for (GoodsPreOrdered pd : allGoods) {
            String log = "Name: " + pd.getProduct() + "\t\tNumber: " + pd.getNumber() + pd.getUnit()
                    + "\t\tCost: " + decimalFormat.format(pd.getCost())
                    + "\t\tTotal: " + numberFormat.format(pd.getTotal())
                    + "\t\tDate of Purchase: " + pd.getDate() + "\t\tForeign Key: " + pd.gettForeignKey();
            Log.d("Insert: ", log + pd.getCost());
            goodsSale.add(log);
            count++;
        }

        listDataHeader.add("Goods Pre Ordered: " + count);
