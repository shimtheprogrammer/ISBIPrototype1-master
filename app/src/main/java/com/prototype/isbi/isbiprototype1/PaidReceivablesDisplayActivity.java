...
public class PaidReceivablesDisplayActivity extends AppCompatActivity
    ...
    PaidReceivablesHandler ppdb = new PaidReceivablesHandler(this);
    ...
    private void prepareListData() {
        ...
        List<String> receivable = new ArrayList<String>();

        List<PaidReceivable> allReceivable = ppdb.getAllPaidReceivable();

        int count = 0;
        for (PaidReceivable pd : allReceivable) {
            String log = "Transaction Id: " + pd.getID() + " \t\tFrom: " + pd.getFrom()
                    + "\t\tTotal: " + numberFormat.format(pd.getTotal()) + "\t\tDate: " + pd.getDate()
                    + "\t\tOriginale Date: " + pd.getOriginalDate() + "\t\tPayment Method: " + pd.getPayment();
            Log.d("Insert: ", log);
            receivable.add(log);
            count++;
        }

        listDataHeader.add("Paid Receivables: " + count);

        listDataChild.put(listDataHeader.get(0), receivable); // Header, Child data
    }
