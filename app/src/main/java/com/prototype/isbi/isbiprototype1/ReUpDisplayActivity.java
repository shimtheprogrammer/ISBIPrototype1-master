...
public class ReUpDisplayActivity extends AppCompatActivity
    ...
    private void prepareListData() {
        ...
        List<ReUp> allExpenses = rudb.getAllReUp();

        int count = 0;
        for (ReUp pd : allExpenses) {
            String log = "Id: " + pd.getID() + " \t\tInfo: " + pd.getInfo()
                    + "\t\tTotal: " + numberFormat.format(pd.getTotal()) + "\t\tPayment Method: " + pd.getPayment()
                    + "\t\tDate: " + pd.getDate() + "\t\tForeign Key: " + pd.gettForeignKey();
            Log.d("Insert: ", log);
            expenses.add(log);
            count++;
        }

        listDataHeader.add("Upgrades on Buidings: " + count);
